...
func CmdListVSOperatorAuthorizations() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "[MOD-DE-QRY-2] List VS operator authorizations. Optionally filter by authority and/or VS operator address.",
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
				return err
	...
	cmd.Flags().String("authority", "", "filter by the authority group that granted the authorization")
	cmd.Flags().String("vs-operator", "", "filter by the VS operator account that received the authorization")
	cmd.Flags().Uint32("limit", 64, "maximum number of results (1-1024, default 64)")
